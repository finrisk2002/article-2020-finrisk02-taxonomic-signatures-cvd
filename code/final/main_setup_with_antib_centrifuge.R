# on FIMM Atlas:
# to symlink data directories etc to "/csc/fr_metagenome/microbiome_scratch/scratch/data_<you>"

# other directories from the template (not all are used)
analysis_date <- paste0(Sys.Date())
analysis_ident <- "centrifuge_withantibiot"
...
phylum_level_phyloseq_path <- paste0(input_datadir,
    "phfinrisk_centrifuge_phylum_ok_drop50k_2018-11-30.RDs")
genus_level_phyloseq_path <- paste0(input_datadir, 
    "phfinrisk_centrifuge_genus_d50k_2019-06-02_rmvg.rds")
species_level_phyloseq_path <- paste0(input_datadir,
    "phfinrisk_centrifuge_species_ok_drop50k_2018-11-30.RDs")
network_identities_path <- paste0(datadir,"/net_components.rds") # if missing, run compute_spieceasi_cox_figures.R
functional_activities_path <- paste0(input_datadir, 
    "function_activity_norxj01.rds")
ko_metabolite_terms_path <- paste0(input_datadir,
    "ko_metabolite_terms.txt")
